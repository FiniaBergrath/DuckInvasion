<p align="center">
<a href="https://github.com/user-attachments/assets/c291045f-6a88-4256-b941-d8633d9c57e9">
<img width="100%" alt="duckInvasionBanner" src="https://github.com/user-attachments/assets/b629e0a8-3fa8-4460-b464-0f9949a82b05" /> 
</a>
</p> 
<h3 align="center"> The galaxy has seen many threats. None of them were this hungry.</h3>


---

<p>
Duck Invasion ist ein Singleplayer-Spiel, das im Rahmen eines Informatikprojekts an der Hochschule Düsseldorf entstanden ist.

Über einen Zeitraum von vier Monaten haben wir (Nick, Lion, Walter, Maik und Finia) dieses Spiel gemeinsam mit der Game Engine Godot entwickelt.
Dieses Repository enthält die Executables, sowie weitere Materialien aus der Projektabgabe.
</p>

## DIE STORY
<a href="https://github.com/user-attachments/assets/c291045f-6a88-4256-b941-d8633d9c57e9">
<img width="100%" alt="gameInfos" src="https://github.com/user-attachments/assets/b3017bb1-763e-4e98-9fc4-4d453fe3044d" />
</a>

## DAS SPIEL
Duck Invasion vereint einen Mix aus verschiedenen Gameplay-Elementen. Je nach Level erwartet einen eine Mischung aus Jump & Run, Target Shooting und Horror.
Die insgesamt fünf Level wurden jeweils von einem Mitglied unseres Teams entwickelt und schließen alle mit einem Bosskampf ab. 
<p> <b>Alle Level verbindet ein gemeinsames Ziel:</b> <br>
So schnell wie möglich die Raumstation zu durchqueren und dabei eine eigene Bestzeit aufzustellen.</p>
<a href="https://github.com/user-attachments/assets/c291045f-6a88-4256-b941-d8633d9c57e9">
<img width="100%" alt="levelÜbersicht" src="https://github.com/user-attachments/assets/de9fa838-9417-428f-bb18-bb474aff7487" />
</a>


## HERUNTERLADEN & STARTEN

1. Lade dir den 'Executable'-Ordner als '.zip'-Datei herunter und entpacke sie in einen Zielordner
2. Navigiere in den Ordner und starte die entsprechende Datei für dein Betriebssystem:
   - Windows: 'DuckInvasion.exe'
   - Linux: 'DuckInvasion.sh' oder DuckInvasion.x86_64'
3. Viel Spaß beim Spielen!

<p align="center">
<a href="https://github.com/user-attachments/assets/c291045f-6a88-4256-b941-d8633d9c57e9">
<img width="414" height="204" alt="steuerung" src="https://github.com/user-attachments/assets/8f348737-be91-43f4-a438-79b82135cc08" />
</a>
</p>

## ERWORBENE FÄHIGKEITEN

Ein Spiel innerhalb eines Semesters von Grund auf zu entwickeln, bedeutet ein bisschen von allem zu können:

<img width="20" height="20" align="absmiddle" alt="duck" src="https://github.com/user-attachments/assets/93604010-3843-4a36-8b3c-98bea39914ab" />
Game Design & Gameplay Mechaniken <br>
<img width="20" height="20" align="absmiddle" alt="duck" src="https://github.com/user-attachments/assets/93604010-3843-4a36-8b3c-98bea39914ab" />
GDScript & Godot Engine <br>
<img width="20" height="20" align="absmiddle" alt="duck" src="https://github.com/user-attachments/assets/93604010-3843-4a36-8b3c-98bea39914ab" />
Visual Design & 3D Modellierung mit Blender <br>
<img width="20" height="20" align="absmiddle" alt="duck" src="https://github.com/user-attachments/assets/93604010-3843-4a36-8b3c-98bea39914ab" />
Gegnerverhalten <br>
<img width="20" height="20" align="absmiddle" alt="duck" src="https://github.com/user-attachments/assets/93604010-3843-4a36-8b3c-98bea39914ab" />
Timer & Highscores <br>
<img width="20" height="20" align="absmiddle" alt="duck" src="https://github.com/user-attachments/assets/93604010-3843-4a36-8b3c-98bea39914ab" />
Debugging von Dingen, die eigentlich hätten funktionieren müssen...

## FAQ

**Warum Enten?** <br>
Enten sind süß!

**Warum sind sie hangry?** <br>
Sie haben nicht gefrühstückt.

**Ist das ein Bug oder ein Feature?** <br>
JA.

**Warum hilft mir mein Companion nicht beim Kämpfen?** <br>
Er hat zu viel gefuttert und ist jetzt im Food-Coma.

<p align="center">
<a href="https://github.com/user-attachments/assets/c291045f-6a88-4256-b941-d8633d9c57e9">
<img width= "35%" alt="hinweis" src="https://github.com/user-attachments/assets/bb1abe77-bc6d-4b65-80f1-1e0e6361b3b6" />
</a>
</p>

---
<h5 align= "center" >HSD Informatikprojekt SoSe24 NICK, LION, MAIK, WALTER, FINIA | MADE WITH GODOT </h5>

---

